...
export default function EditPromoStep4() {
    ...
    const onSubmit = async (form: FormValues) => {
        ...
        try {
            await updatePromoRewards(promoId as string, payload);
            setData((prevData) => ({ ...prevData, incentives: form.items }));

            toast.success(
                t("promo.promo_updated_successfully"),
                { autoClose: 4000 }
            );

            setTimeout(() => {
                router.push(`/dashboard/airline/promos/${promoId}/view`);
            }, 1000);
        } catch (e: unknown) {
...
function AudienceCard({ idx, control, removeAudience, audienceOptions }: CardProps) {
    ...
    useEffect(() => {
        ...
        if (current.rewards && Array.isArray(current.rewards)) {
...
function ThresholdReward({
    ...
    const { data } = usePromoWizard();
    const agencyId = useWatch({
        control,
        name: `items.${audIdx}.rewards.${rwIdx}.agencyId`,
    });

    const agencyName = data.targetAudiences?.find(audience => 
        audience.agencies?.some(agency => agency.value === agencyId)
    )?.agencies?.find(agency => agency.value === agencyId)?.label || agencyId;
        ...
        <div className="space-y-4 bg-gray-50 rounded-lg p-5">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                <p className="text-sm font-medium text-blue-800">
                    {t("promo.target_agency")}: {agencyName}
                </p>
            </div>
